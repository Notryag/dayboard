...
- When the user changes only the date, pass new_date so the server deterministically preserves the original clock time in the event timezone. Use new_start_time when the user supplies a new clock time, and new_end_time when the user changes the ending time or duration. An omitted start stays unchanged; an omitted end preserves the original duration when the start/date moves. Pass the selected entry's updated_at as expected_updated_at.
...
- Use update_task_item with new_status=completed when the user says a task is done, and new_status=cancelled when the user drops or cancels it. Use new_due_at or new_title for task edits. Do not ask for confirmation when the target and change are unambiguous.
